...
Datum pg_sphinx_replace(PG_FUNCTION_ARGS)
{
  ...
  size_t len = ARR_DIMS(input)[0] / 2;
  PString *columns = palloc(sizeof(PString) * len);
  PString *values  = palloc(sizeof(PString) * len);

  ArrayIterator iter = array_create_iterator(input, 0);
  Datum value;
  bool isnull;
  size_t i = 0;
